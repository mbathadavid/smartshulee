...
class Trs extends Trs_Controller
{
  ...
  public function record()
  {
    ...
    if ($this->input->post()) {
        ...
        $students = $this->boarding_m->get_students_by_stream($post->class);
     
    }

    // Check whether logged in poersin is class teacher

    $check = $this->boarding_m->myclasses();
  ...
  function submit_ratings() {
    ...
    foreach ($rating as $st => $rt) {
      ...
      $check = $this->boarding_m->checkratings($st,$this->input->post('term'),$this->input->post('year'), $this->input->post('item'));

      if ($check) {
        $form_data['modified_by'] = $user->id;
        $form_data['modified_on'] = time();

        $done = $this->boarding_m->update_attributes($check->id,$form_data);
          if ($done) {
            $kkk++;
          } else {
            $kk++;
  ...
  function submit_comments() {
      ...
      foreach ($comments as $st => $com) {
        $form_data = array(
            ...
            'year' => $post->year,
            'comment' => $com,
            'parent_name' => $post->parent[$st]
        );

        $check = $this->boarding_m->checkcomments($st,$post->term,$post->year);

        if ($check) {
          $form_data['modified_by'] = $user->id;
          $form_data['modified_on'] = time();
  
          $done = $this->boarding_m->update_comments($check->id,$form_data);
            if ($done) {
              $kkk++;
            } else {
              $kk++;
          ...
          $form_data['created_by'] = $user->id;
          $form_data['created_on'] = time();
          $ok = $this->boarding_m->create_comment($form_data);
  ...
  // die;
    $mess = $k.' Records Created. '.$kkk.' Records Updated. '.$kk.' Failed';
    $this->session->set_flashdata('message', array( 'type' => 'success', 'text' => $mess));
    redirect('boarding/trs/comments');
}
