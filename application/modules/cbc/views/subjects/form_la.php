...
<div id="tasks" v-cloak>
    ...
    <div class="card-box hidden-print">
        <h4 class="m-t-0 m-b-10 header-title text-center text-uppercase">Add Topics/Sub Strands</h4>
            ...
            <div class="row">
                ...
                <div class=" pull-right">
                    <a href="javascript:" @click="expand()" class=" btn btn-white btn-dim btn-outline-light" >
                        <i class="glyphicon glyphicon-cog"></i><span ref="excol">&nbsp; &nbsp; {{ sf? 'Hide Tasks': 'Show Tasks' }}</span>
                    ...
                    <div class="tasks-list-w">
                        ...
                        foreach ($post->topics as $s)
                        {
                            ...
                            <div class="pb">
                                <div class="tl-header">
                                    <?php echo '1.' . $j . " &nbsp; &nbsp; &nbsp;" . $s->name; ?> 
                                    <a class="pull-right prl btn-link" href="javascript:" @click="manage_tasks(<?php echo $s->id ?>)"><strong>+Add Tasks </strong></a>
                                </div>
                                <?php
                                if (count($s->tasks))
                                {
...
<style>
    ...
    .btn-outline-light{border-color:#dbdfea;color:#526484;}
...
<script>
    ...
    const tsk = new Vue({
        ...
        methods: {
            ...
            {
                axios.post("<?php echo base_url('admin/cbc/update_task') ?>" + "/" + task.id, {name: task.name})
            ...
            save_tasks: function (id)
            {
                ...
                axios.post("<?php echo base_url('admin/cbc/tasks') ?>" + "/" + id, {tasks: this.tasks})
                        .then(function (response)
                        {
...
<style>
    a.btn-link { color: #047bf8 !important;}
    ...
    .subby{background-color:#fff;-webkit-box-flex:1;-ms-flex:1;flex:1;}
    ...
    .tl-header{font-size: 13px; font-weight: 600; margin-top: 12px;}
    .tasks-header-w{border-bottom:1px solid rgba(0, 0, 0, 0.1);position:relative;margin-bottom:10px;}
    .tasks-header-w .tasks-header{width: 100%;padding-left: 4px; display:inline-block;margin-bottom:0px;}
    .tasks-list-w{padding-left: 24px;}
    @media (min-width: 768px) and (max-width: 1024px){
        .subby{padding:30px;}
    }
    @media (max-width: 768px){
        .subby{padding:30px;}
        .all-tasks-w{padding-left:5px;}
        .tasks-header-w{padding-left:20px;}
        .subby-header{font-size:1.25rem;}
        .all-tasks-w{padding-top:0px;padding-right:0px;}
    }
    @media (max-width: 767px){
        .subby{padding:30px 20px;}
        .subby-header{font-size:1.25rem;}
        .all-tasks-w{padding-top:0px;padding-right:0px;}
    }
...
<style>
    .mni{color: #555555b8;}
    .icosg-arrow-right4{color: #ccc !important;}
    input.form-control { height: 36px!important;}
    label{font-size:14px;color:#313a46;letter-spacing:0.01em;}
    .form-controls{border:1px solid #dadada;border-radius:4px;padding:7px 12px;height:38px;max-width:100%;font-size:14px;-webkit-box-shadow:none;box-shadow:none;-webkit-transition:all 300ms linear;-moz-transition:all 300ms linear;-o-transition:all 300ms linear;transition:all 300ms linear;}
    .form-control:focus{border:1px solid #afafaf;-webkit-box-shadow:none;box-shadow:none;outline:0!important;}
